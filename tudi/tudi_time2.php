...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {	
	...
	if (!empty($trimmed['start_time2'])) {
		$start_time2 = mysqli_real_escape_string ($conn, $trimmed['start_time2']);
	} else {
		$start_time2 = '';
	}
	
	if (!empty($trimmed['end_time2'])) {
		$end_time2 = mysqli_real_escape_string ($conn, $trimmed['end_time2']);
	} else {
		$end_time2 = '';
	}
	...
	if($start_time2 && $end_time2){
		...
		$sql = " chengjiao_time >='".$start_time2."' AND chengjiao_time<='".$end_time2."' ";
		
	}else if($start_time2){   
	
		$sql = " chengjiao_time ='".$start_time2."' ";
		
	}else if($end_time2){
		
		$sql = " chengjiao_time ='".$end_time2."' ";
